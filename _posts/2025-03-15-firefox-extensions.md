...
* [Yang!](https://addons.mozilla.org/en-US/firefox/addon/yang-addon/): An extension that brings client-side search bangs (which first appeared in [DuckDuckGo](https://duckduckgo.com/bangs)). You can keep using your favorite search engine (e.g., Google), it avoids an extra server roundtrip (which may be problematic for privacy as well), it's faster, and it's supported on Android as well.
